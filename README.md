...
- **Motion detection via baseline subtraction** (on by default; disable
  with `--no_motion`): flags anything different from a pre-recorded
  empty-room baseline, catches "moved chair" / "object dropped" cases
  that pure semantic segmentation misses.
